...
namespace ImGui
{
    ...
    int Curve(const char *label, const ImVec2& size, const int maxpoints, ImVec2 *points, std::vector<float>& weights, int& item)
    {
        ...
        // smooth curve
        //enum { smoothness = weights.size() }; // the higher the smoother
        int smoothness = static_cast<int>(weights.size());
        ...
        RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + style.FramePadding.y), bb.Max, "1,1", NULL, NULL, { 1.0f,0.0 });
        RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + style.FramePadding.y), bb.Max, "0,0", NULL, NULL, { 0.0f,1.0 });
        RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + style.FramePadding.y), bb.Max, "1,0", NULL, NULL, { 1.0f,1.0f });
        Text(str);
