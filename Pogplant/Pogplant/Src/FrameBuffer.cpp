...
namespace Pogplant
{
	...
	void FrameBuffer::ResizeFrameBuffer(BufferType _BufferType, const float _Scale)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)(Window::m_Width * _Scale), (GLsizei)(Window::m_Height * _Scale), 0, GL_RGBA, GL_FLOAT, NULL);
	}
	...
	bool FrameBuffer::InitGBuffer()
	{
		...
		unsigned int* gEmissive = &FBR::m_FrameBuffers[BufferType::G_EMISSIVE_BUFFER];
		...
		unsigned int* gCanvas = &FBR::m_FrameBuffers[BufferType::G_CANVAS_BUFFER];
		glGenTextures(1, gCanvas);
		glBindTexture(GL_TEXTURE_2D, *gCanvas);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Window::m_Width, Window::m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
